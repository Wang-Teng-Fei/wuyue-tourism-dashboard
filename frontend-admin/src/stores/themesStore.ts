...
export const useThemesStore = defineStore('themes', () => {
  ...
  const toggleTheme = (isTheme: boolean, mouseX: number, mouseY: number) => {
    ...
    // 抽离设置 clip-path 的逻辑
    const setClipPath = (oldSize: string, newSize: string) => {
      document.documentElement.style.setProperty('--clip-path-old', `circle(${oldSize} at ${xPercent}% ${yPercent}%)`);
      document.documentElement.style.setProperty('--clip-path-new', `circle(${newSize} at ${xPercent}% ${yPercent}%)`);
    };
